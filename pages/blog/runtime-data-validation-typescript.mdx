...
Although typescript has types, javascript does not. And since typescript transpiles into javascript to run at the end of the day,
...
and work accordingly. This is a pattern observed in [haskell](http://learnyouahaskell.com/for-a-few-monads-more#error) and rust (Rust has
...
I suggest reading [a post about decorators in python](/blog/decorators-python). I covers a similar concept using python).

This might seem like a lot, and it is. There is actually a better way of doing this `decodeRawDataWith` function using `pipe`. I am using
...
Thats it! Now with just a function call, data can be validated at runtime using io-ts. 
