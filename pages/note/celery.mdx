...
This is key, `verified_check` and `send_email` are a chain. They *need to be executed in sequence*. But the 
*chains should be executed in parallel*. The vocabulary is very important.
...
- To execute a function as a *celery* task instead of a function, add `.delay` to it and make sure the function has the
  decorator.
...
- Celery also has a really cool feature of [auto
  retry](http://docs.celeryproject.org/en/master/userguide/tasks.html#automatic-retry-for-known-exceptions), you can
  dump some params in the decorator and it takes care of it automatically. `@app.task(autoretry_for=(Exception,),
  retry_backoff=2)`
