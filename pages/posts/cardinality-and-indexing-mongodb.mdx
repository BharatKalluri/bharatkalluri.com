...
> The total number of distinct elements in a set is described as the Cardinality of the set
...
If a boolean field is indexed, there is not much the index will improve on terms of performance. Imagine a column in
...
This is one more index types which is really popular (for a good reason), the idea is that sparse index allows to create
an index where the value of the field is not undefined. This also helps when you have an optional unique fields. This
helps us if the index is on a field where the majority of the fields are undefined.

### Compound indexes

This is the most common, well established pattern from SQL. The idea is that, based on your queries, create a compound
index with the field which has the max cardinality first and the least cardinality to the last. So, whenever a query is
run, it will narrow down the funnel by filtering out the majority of the rows (since the first piece in the compound
index has the highest cardinality) and progressively narrows down on the result.
