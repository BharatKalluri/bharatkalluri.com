...
There are 3 rules an SRT file is expected to follow
...
which analyzed around 50,000 srt files from open subtitles. Here are some highlights from that article
...
- Right after this, there will be __one or more__ lines of content
...
- After that, we first filter all the lines which just contain a number. These are the lines which contain ID's
    we will be populating them manually later on.
- Next, filter on empty lines. To skip all the new lines which are there to signify cue block end.
...
when there is no standard. Currently [there are over __50 competing formats!__ ](https://wiki.whatwg.org/wiki/Timed_track_formats)
