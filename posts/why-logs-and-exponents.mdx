...
The way to calculate growth rate is also fascinating. The formulae for calculation is

```python
import math
# growth_rate being 0 to 1, where 1 is 100%
growth_rate = 1
# t being the time interval we want to get value for
t = 10
# starting point of the growth
initial_value = 1

value_at_time_t = initial_value * math.pow(
    (1+growth_rate)
, t)
```
...
That is saying, how many 2's does it take to multiple to make 1073741824. That is what a log is.

Apart from this neat data compression feature, logs make it extremely easy to perform math with large numbers given its properties.
