...
Now open `http://localhost:8080` to visit openlibrary locally. To login, use `openlibrary@example.com` and password
as `admin123`. You should now be able to edit books, add books etc. This will give you admin privileges.
...
```
After this, search for `digital fortress` in the locally running open library instance and it should show up!


### Why is it that the reading log is not working on my local OL(Openlibrary) instance?
