...
## The journey of an database write
...
If the idea is safety, then have `w` set to `majority` and set `j` to true. To make sure the write goes through and the chance of something going wrong is extremely small.

Thats a gist of write concerns in MongoDB.